...
public class MyView extends ImageView implements ViewTreeObserver.OnGlobalLayoutListener,
        ...
        mGestureDetector = new GestureDetector(context, new GestureDetector
            ...
            public boolean onDoubleTap(MotionEvent e) {
                ...
                if (getScale() < mMidScale) {
                    //放大
//                    mMatrix.postScale(mMidScale / getScale(), mMidScale / getScale(), e.getX(),
// e.getY());
                    postDelayed(new AutoScaleRunnable(mMidScale, e.getX(), e.getY()), 16);
                    isAutoScale = true;
                } else {
                    //缩小
//                    mMatrix.postScale(mInitScale / getScale(), mInitScale / getScale(), e.getX
// (), e.getY());
    ...
    class AutoScaleRunnable implements Runnable {
        ...
        public void run() {
            ...
            if ((currentScale < targetScale && tempScale > 1.0f) || (currentScale > targetScale
                    && tempScale < 1.0f)) {
                postDelayed(this, 15);
            } else {
